...
## 2026-08-20 營區速查顯示三種狀態（Claude）

### 背景

`campcool-bot` 端把營區資料庫從 710 擴充到 1101 筆（見該 repo 的 AI-README §6.1），
新增花蓮、台東、高雄、屏東、台南、彰化、基隆、嘉義市、雲林、台北等原本全空白的縣市。

### 發現

**擴充後網站只顯示 276 筆，新增的 825 筆完全看不到。** 業主查「二林中科露營農場」
得到「沒有符合的營區」。

`index.html` 有三處把 `allows_ac` 為 NULL 的濾掉：

| 位置 | 原本行為 |
|---|---|
| `loadSitesStats` | 統計只加總 `allows_ac === 1 或 0` → 顯示「已確認 276」 |
| `searchSites` 的 filter | `if (s.allows_ac !== 1 && s.allows_ac !== 0) return false;` → **列表直接不顯示** |
| 搜尋建議清單 | 同樣過濾 → 打字沒有提示 |

原始註解寫「未確認的不對客戶顯示」，**當時是合理的**：那時「未確認」等於完全沒資料。
但現在這些營區有地址、電話、營規逐字原文與營業季節，藏起來只會讓客戶以為沒收錄。

### 變更

- 統計改為「收錄 N 個營區（可帶／不可帶／待確認）」，三種都顯示
- 列表與搜尋建議不再過濾 NULL
- 狀態下拉新增「❓ 要再跟營主確認」，filter 加 `status === 'ask'` 分支
- 每筆顯示狀態文字，**取自後端的 `ac_status`**（`s.ac_status || 後備推導`）。
  前端不再自行判斷，避免與 bot、LINE 回覆出現三套說法
- `ac_probable === 1` 者額外顯示 `ac_advice`（營規只禁高耗電、未點名冷氣時，
  說明移動式冷氣 425-530W 通常不在此列，建議先問營主）
- 補上 `aircon_policy`（營規原文）、`phone`、`season_open` 三個欄位

### 驗證

`/public/sites` 不是本 repo 控制的，所以**不能只跑 validate**——那只驗靜態內容。
本輪的做法是**把 `index.html` 裡實際的 filter 程式碼用正則抽出來，
餵真實 API 回應執行**，而不是另外重寫一份邏輯來測（重寫的版本會與真正跑的程式脫節）：

```
二林中科：API 1 筆 → 顯示 1（待確認 1）
50 筆樣本：全數顯示（可帶 2／不可 14／待確認 34），三種篩選各自正確
```

`node scripts/validate-site.mjs --selftest` 與 20 項站台驗證亦通過。

### 後續

- 這類「顯示層過濾掉後端資料」的問題，靜態驗證與看原始碼都抓不到，只有實際
  對真實 API 跑一次才會發現。建議把它視同「規則二」的第五類變更：
  **凡是改動 `/public/*` 資料的呈現或過濾條件，必須用真實 API 回應驗證一次。**
- `ac_status` 是後端 SQL 產生的唯一事實來源。日後若要改答案的措辭，
  改 `campcool-bot` 的 `/public/sites`，不要在前端各自加工。
